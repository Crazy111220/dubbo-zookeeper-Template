...
function loadBbsSort(jsonEntity){
  //方法级渲染
  table.render({
   elem: '#listTable'
   ,url: '/bbssort/listByPramas'
   ,width:1560
   ,height: 480
   ,page:true
   ,even:true
   ...
   ,cols: [[
     ...
     ,{field:'name', title: '帖子类型名称',align:'center', width:160, sort: true}
  ...
  table.on('tool(list)', function(obj){
    ...
    if(obj.event === 'show'){
      ...
      layer.confirm('真的删除行么', function(index){
        ...
        czBbsSort("/bbssort/delete",jsonEntity,obj);
        layer.close(index);
      });
      ...
      layer.open({
		...
		anim:4,
        shade:0.6,
  		content:$("#updBbsSortDiv"),
  		success:function(){
        form.val("updBbsSortForm", {
    "name": data.name,
        });

  		          //监听提交
			  form.on('submit(submitUpdBbsSort)', function(FormData){
			    var jsonEntity = FormData.field
			    jsonEntity.id = data.id;
			    czBbsSort("/bbssort/updT",jsonEntity,obj);
			    layer.closeAll();
			    return false;
			  });
  		}
    ...
    $("#delBatchBbsSort").click(function(){
	   ...
	   if(data.length<1){
          layer.msg("请选择数据！！！",{"icon":3,anim:6,shade:0.6,"time":2000});
           ...
           layer.confirm("你确定要删除所选中的数据吗？？？", {icon: 7, title: '提示'}, function (index) {
               ...
               delManyBbsSort("/bbssort/deletes", params);
               flush(currentPage);
               layer.close(index);
           });
       }
  ...
  $("#saveUI").click(function(){
	  layer.open({
	    ...
	    shade:0.6,
		  ...
		  form.on('submit(submitUpdBbsSort)', function(FormData){
            var jsonEntity = FormData.field
		    czBbsSort("/bbssort/saveT",jsonEntity);
		    layer.closeAll();
		    return false;
		  });
  		}
  ...
  //普通增删改操作
  function czBbsSort(url,jsonEntity,obj){
	  $.ajax({
		   	type:"post",
		   	url:url,
		   	data:jsonEntity,
  ...
  //批量删除
  function delManyBbsSort(url,ids){
	  $.ajax({
			type:"post",
			url:url,
			data:{'ids':ids},
